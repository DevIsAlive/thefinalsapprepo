...
namespace RapidOcrNet
{
    public sealed class TextClassifier : IDisposable
    {
        ...
        private readonly float[] _meanValues = new float[] { 127.5F, 127.5F, 127.5F };
        private readonly float[] _normValues = new float[] { 1.0F / 127.5F, 1.0F / 127.5F, 1.0F / 127.5F };
        ...
        public Angle GetAngle(SKBitmap src)
        {
            ...
            try
            {
                using (IDisposableReadOnlyCollection<DisposableNamedOnnxValue> results = _angleNet.Run(inputs.ToArray()))
                {
                    float[] outputData = results[0].AsEnumerable<float>().ToArray();
        ...
        private static Angle ScoreToAngle(float[] srcData, int angleColumns)
        {
            int angleIndex = 0;
