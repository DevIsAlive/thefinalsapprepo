...
namespace RapidOcrNet
{
    public sealed class TextDetector : IDisposable
    {
            ...
            float unClipRatio)
        {
            ...
            IEnumerable<NamedOnnxValue> inputs = new NamedOnnxValue[]
            {
            ...
            try
            {
                using (IDisposableReadOnlyCollection<DisposableNamedOnnxValue> results = _dbNet.Run(inputs.ToArray()))
                {
        ...
        {
            int[] v = Array.ConvertAll(array, c => (int)c);
        ...
        private static List<TextBox> GetTextBoxes(DisposableNamedOnnxValue outputTensor, int rows, int cols, ScaleParam s, float boxScoreThresh, float boxThresh, float unClipRatio)
        {
            ...
            byte[] thresholdMat = new byte[predData.Length];
            byte[] cbufMat = new byte[predData.Length];
...
//#endif
            ...
            using (SKImage predImage = SKImage.FromPixelCopy(gray8, cbufMat))
            {
                for (int i = 0; i < contours.Length; i++)
                {
                    ...
                    SKPoint[] clipMinBox = GetMiniBox(clipBox, out maxSide);
        ...
        private static SKPoint[] GetMiniBox(SKPoint[] contours, out float minEdgeSize)
        {
            ...
            minEdgeSize = (float)Math.Min(width, height);
...
//#endif
        ...
        private static SKPoint[] Unclip(SKPoint[] box, float unclipRatio)
        {
            ...
            float area = Math.Abs(SignedPolygonArea(box));
