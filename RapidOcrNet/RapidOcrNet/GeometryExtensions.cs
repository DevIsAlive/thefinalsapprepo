...
namespace RapidOcrNet
{
    internal static class GeometryExtensions
    {
        ...
        private static float polarAngle(SKPoint point1, SKPoint point2)
        {
            ...
            return (float)(Math.Atan2(point2.Y - point1.Y, point2.X - point1.X) % Math.PI);
        }

        /// <summary>
        /// Algorithm to find the convex hull of the set of points with time complexity O(n log n).
        /// </summary>
        public static List<SKPoint> GrahamScan(SKPoint[] points)
        {
            ...
            var sortedPoints = new SKPoint[groups.Length];
            ...
            try
            {
                for (int i = 0; i < groups.Length; i++)
                {
                    ...
                    else
                    {
                        ...
                        sortedPoints[i] = maxPoint;
                    }
                }

                if (groups.Length < 2)
                {
                    return new List<SKPoint> { P0, sortedPoints[0] };
                }
                ...
                return new List<SKPoint>(stack);
            }
            finally
            {
                // No need to return sortedPoints as it's now part of the stack
            }
        }
        ...
        /// </param>
        private static SKPoint[] ParametricPerpendicularProjection(SKPoint[] polygon)
        {
            if (polygon.Length == 0)
            {
        ...
        public static void GetSize(SKPoint[] points, out float width, out float height)
        {
            ...
            width = (float)Math.Sqrt((bottomLeft.X - bottomRight.X) * (bottomLeft.X - bottomRight.X) + (bottomLeft.Y - bottomRight.Y) * (bottomLeft.Y - bottomRight.Y));
            height = (float)Math.Sqrt((bottomLeft.X - topLeft.X) * (bottomLeft.X - topLeft.X) + (bottomLeft.Y - topLeft.Y) * (bottomLeft.Y - topLeft.Y));
        }
    }
}
